...
#
# One step: render the bundle's obsidian axis into THIS account's vaults right
...
# -up for a pack installed by someone else). The axis also enables the snippet
# by default in fresh vaults (its guarded appearance.json write, three-state
# rule — see pack-office.md). No root-only steps, so it works identically under
# rootless `w-pack setup`.
#
...
# ── 2. What the user may still want to know, said out loud ─────────────────────
...
info "  Obsidian was running during install? The enable self-heals at the next"
info "  login or theme switch with the app closed."
