// W Linux — Hub Date & Time section (the System panel's "Date & Time" tab).
// The timezone, the NTP on/off switch and the NTP server set, all fronted by the w-time
...
//
// This was a root-level Hub panel of its own until the menu reorganisation. Date & Time
// sits under the System screen in every reference settings app (macOS General → Date &
// Time, GNOME System → Date & Time, Windows Time & language), and System already owned
// the neighbouring system-language row.
//
// Both signals below are re-emitted by SystemPanel: the Hub wires a panel's optional
// signals with `Connections { target: panelLoader.item }`, which sees only the TOP-level
// panel — a signal emitted from inside a nested Loader never reaches it.
//
...
//
// Loaded by SystemPanel (Loader{source:"DateTimeSection.qml"}); shared hub components
// (SelectRow, HubSection, HubRow) come via `import qs.modules.hub`.
import QtQuick
...
Column {
    ...
    // Drill into the timezone picker; privileged actuation handed to the Hub. Both are
    // forwarded by SystemPanel (see the header note on the Connections target).
    ...
    // Field names: "tz" | "ntp" | "servers" — all three always present.
    property string focusedField: ""
    readonly property var fields: ["tz", "ntp", "servers"]
    function rowItem(field) {
        if (field === "tz") return tzRow;
        if (field === "ntp") return ntpRow;
        if (field === "servers") return srvRow;
        return null;
    }
    ...
    HubRow {
        ...
        focused: root.focusedField === "tz"
        onActivated: root.navigate("system.timezone")
    }
    ...
    SelectRow {
        ...
        focused: root.focusedField === "ntp"
        onActivated: if (root.menuLayer) root.menuLayer.openMenu(ntpRow, root.ntpOptions, root.ntp, (id) => {
    ...
    SelectRow {
        ...
        focused: root.focusedField === "servers"
        onActivated: if (root.menuLayer) root.menuLayer.openMenu(srvRow, root.serverOptions, root.serversSel, (id) => {
