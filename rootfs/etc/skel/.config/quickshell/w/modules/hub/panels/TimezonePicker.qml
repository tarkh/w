// W Linux — Hub timezone picker (drilled from DateTimePanel).
// A searchable catalogue of IANA timezones (`w-time list-zones --porcelain`), each shown
// as its current UTC offset + the zone name, so the user can find and pick the right one.
// Picking one sets the system timezone via the privileged path — pkexec +
// com.w.hub.actuate, capability timezone-set → `w-time set-zone` — handed to the Hub as
// runPrivileged, which suspends for the polkit prompt and restores on exit; on completion
// it pops back to the Date & Time panel.
//
// Fills the card height and scrolls internally; too large for the small HubMenu, hence a
// full drill-in screen (mirrors LocalePicker). Loaded via HubRegistry; shared bits via
// qs.modules.hub.
import QtQuick
...
FocusScope {
    ...
    property var zones: []                     // [{ offset: "+03:00", zone: "Europe/Moscow" }]

    function filtered() {
        const q = root.query.trim().toLowerCase();
        if (!q) return root.zones;
        return root.zones.filter(z => z.zone.toLowerCase().indexOf(q) >= 0 || z.offset.indexOf(q) >= 0);
    }
    ...
    Process {
        id: zoneList
        running: true
        command: ["w-time", "list-zones", "--porcelain"]
        stdout: StdioCollector {
            onStreamFinished: {
                const out = [];
                for (const line of (this.text || "").split("\n")) {
                    const m = line.trim().match(/^(\S+)\s+(\S+)$/);   // "<offset> <zone>"
                    if (m) out.push({ offset: m[1], zone: m[2] });
                }
                root.zones = out;
            }
        }
    ...
    function pick(zone) {
        root.runPrivileged(["pkexec", "/usr/lib/w/w-hub-actuate", "timezone-set", zone],
                           () => root.navigateBack());
    }
    ...
    Column {
        ...
        Rectangle {
            ...
            TextField {
                ...
                background: null
                color: Colors.text
                placeholderText: Strings.t("hub.searchTimezone")
                ...
                Keys.onPressed: (e) => {
                    ...
                    case "confirm":
                        if (results.currentIndex >= 0 && results.currentIndex < results.count)
                            root.pick(results.model[results.currentIndex].zone);
                        e.accepted = true;
                        return;
        ...
        ListView {
            id: results
            // Extend into the card's right padding (see LocalePicker); delegates inset 12.
            ...
            delegate: Item {
                ...
                Row {
                    ...
                    Text {
                        width: 56
                        text: dele.modelData.offset
                        color: dele.current ? Colors.accentFg : Colors.muted
                        font.family: Fonts.mono; font.pixelSize: 13
                        Behavior on color { ColorAnimation { duration: Motion.fast } }
                    }
                    Text {
                        text: dele.modelData.zone
                ...
                MouseArea {
                    ...
                    onClicked: root.pick(dele.modelData.zone)
                }
            }
        }
    }
